...
Once installed you can run the Kotha CLI.
...
```bash
kotha -c /path/to/project
```

## Flags 

The Kotha CLI takes the following flags.
...
By default, Kotha runs in interactive mode.

But you can also run Kotha in non-interactive mode by passing a prompt directly as a command-line argument. This is useful for scripting, automation, or when you want a quick answer without launching the full TUI.
...
```bash "-q"
kotha -p "Explain the use of context in Go" -q
```

In this mode, Kotha will process your prompt, print the result to standard output, and then exit. All **permissions are auto-approved** for the session.
...
- `--allowedTools`
  ...
  ```bash "--allowedTools"
  kotha -p "Explain the use of context in Go" --allowedTools=view,ls,glob
  ```
...
- `--excludedTools`
  ...
  ```bash "--excludedTools"
  kotha -p "Explain the use of context in Go" --excludedTools=bash,edit
  ```
