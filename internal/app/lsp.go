...
func (app *App) createAndStartLSPClient(ctx context.Context, provider config.ConfigProvider, name string, command string, args ...string) {
	...
	lspClient, err := lsp.NewClientWithConfig(ctx, provider, logging.GetDefaultLogger(), command, args...)
	if err != nil {
	...
	workspaceWatcher := watcher.NewWorkspaceWatcher(lspClient, provider, logging.GetDefaultLogger())
