...
func HandleRegisterCapability(client *Client, params json.RawMessage) (any, error) {
	var registerParams protocol.RegistrationParams
	if err := json.Unmarshal(params, &registerParams); err != nil {
		client.logger.Error("Error unmarshaling registration params", "error", err)
		return nil, err
	}
	...
	for _, reg := range registerParams.Registrations {
		...
		case "workspace/didChangeWatchedFiles":
			...
			if err != nil {
				client.logger.Error("Error marshaling registration options", "error", err)
				continue
			}

			var options protocol.DidChangeWatchedFilesRegistrationOptions
			if err := json.Unmarshal(optionsJSON, &options); err != nil {
				client.logger.Error("Error unmarshaling registration options", "error", err)
				continue
			}
...
func HandleApplyEdit(client *Client, params json.RawMessage) (any, error) {
	...
	if err != nil {
		client.logger.Error("Error applying workspace edit", "error", err)
...
func HandleServerMessage(client *Client, params json.RawMessage) {
	cnf := client.configProvider.Get()
	var msg struct {
		Type    int    `json:"type"`
	...
	if err := json.Unmarshal(params, &msg); err == nil {
		if cnf.DebugLSP {
			client.logger.Debug("Server message", "type", msg.Type, "message", msg.Message)
		}
	}
}
...
func HandleDiagnostics(client *Client, params json.RawMessage) {
	...
	if err := json.Unmarshal(params, &diagParams); err != nil {
		client.logger.Error("Error unmarshaling diagnostics params", "error", err)
		return
	}
