...
func NewWorkspaceWatcher(client *lsp.Client, configProvider config.ConfigProvider, logger logging.Logger) *WorkspaceWatcher {
...
func (w *WorkspaceWatcher) AddRegistrations(ctx context.Context, id string, watchers []protocol.FileSystemWatcher) {
	cnf := w.configProvider.Get()

	w.logger.Debug("Adding file watcher registrations")
	...
	if cnf.DebugLSP {
		...
		for i, watcher := range watchers {
			w.logger.Debug("Registration", "index", i+1)

			// Log the GlobPattern
			switch v := watcher.GlobPattern.Value.(type) {
			case string:
				w.logger.Debug("GlobPattern", "pattern", v)
			case protocol.RelativePattern:
				w.logger.Debug("GlobPattern", "pattern", v.Pattern)

				// Log BaseURI details
				switch u := v.BaseURI.Value.(type) {
				case string:
					w.logger.Debug("BaseURI", "baseURI", u)
			...
			default:
				w.logger.Debug("GlobPattern", "unknown type", fmt.Sprintf("%T", v))
			}
			...
			w.logger.Debug("WatchKind", "kind", watchKind)
		}
	}

	// Determine server type for specialized handling
	serverName := getServerNameFromContext(ctx)
	w.logger.Debug("Server type detected", "serverName", serverName)
	...
	if shouldPreloadFiles(serverName) || !hasFileWatchers {
		go func() {
			...
			if cnf.DebugLSP {
				w.logger.Debug("Opened high-priority files",
			...
			if filesOpened >= maxFilesToOpen {
				if cnf.DebugLSP {
					w.logger.Debug("Reached file limit with high-priority files",
			...
			err := filepath.WalkDir(w.workspacePath, func(path string, d os.DirEntry, err error) error {
				...
				if d.IsDir() {
					if path != w.workspacePath && shouldExcludeDir(path) {
						if cnf.DebugLSP {
							w.logger.Debug("Skipping excluded directory", "path", path)
						}
			...
			if cnf.DebugLSP {
				w.logger.Debug("Limited workspace scan complete",
					"filesOpened", filesOpened,
					"maxFiles", maxFilesToOpen,
			...
			if err != nil && cnf.DebugLSP {
				w.logger.Debug("Error scanning workspace for files to open", "error", err)
			}
		}()
	} else if cnf.DebugLSP {
		w.logger.Debug("Using on-demand file loading for server", "server", serverName)
	}
}
...
func (w *WorkspaceWatcher) openHighPriorityFiles(ctx context.Context, serverName string) int {
	...
	for _, pattern := range patterns {
		...
		if err != nil {
			if cnf.DebugLSP {
				w.logger.Debug("Error finding high-priority files", "pattern", pattern, "error", err)
			}
			continue
		}
		...
		for _, match := range matches {
			...
			if err != nil || info.IsDir() || w.shouldExcludeFile(fullPath) {
				continue
			}
			...
			if err := w.client.OpenFile(ctx, fullPath); err != nil {
				if cnf.DebugLSP {
					w.logger.Debug("Error opening high-priority file", "path", fullPath, "error", err)
				}
			} else {
				filesOpened++
				if cnf.DebugLSP {
					w.logger.Debug("Opened high-priority file", "path", fullPath)
				}
			}
...
func (w *WorkspaceWatcher) WatchWorkspace(ctx context.Context, workspacePath string) {
	...
	serverName := getServerNameFromContext(ctx)
	w.logger.Debug("Starting workspace watcher", "workspacePath", workspacePath, "serverName", serverName)
	...
	err = filepath.WalkDir(workspacePath, func(path string, d os.DirEntry, err error) error {
		...
		if d.IsDir() {
			...
			if err != nil {
				w.logger.Error("Error watching path", "path", path, "error", err)
			}
		}
	...
	if err != nil {
		w.logger.Error("Error walking workspace", "error", err)
	}
	...
	for {
		...
		case event, ok := <-watcher.Events:
					...
					if info.IsDir() {
						...
						if !shouldExcludeDir(event.Name) {
							if err := watcher.Add(event.Name); err != nil {
								w.logger.Error("Error adding directory to watcher", "path", event.Name, "error", err)
							}
						}
					} else {
			...
			if watched, watchKind := w.isPathWatched(event.Name); watched {
				...
				case event.Op&fsnotify.Create != 0:
					...
					if err != nil {
						w.logger.Error("Error getting file info", "path", event.Name, "error", err)
						return
					}
		...
		case err, ok := <-watcher.Errors:
			...
			w.logger.Error("Error watching file", "error", err)
		}
	}
}
...
func (w *WorkspaceWatcher) matchesPattern(path string, pattern protocol.GlobPattern) bool {
	...
	if err != nil {
		w.logger.Error("Error parsing pattern", "pattern", pattern, "error", err)
		return false
	}
	...
	if err != nil {
		w.logger.Error("Error getting relative path", "path", path, "basePath", basePath, "error", err)
		return false
	}
...
func (w *WorkspaceWatcher) handleFileEvent(ctx context.Context, uri string, changeType protocol.FileChangeType) {
	...
	if changeType == protocol.FileChangeType(protocol.Deleted) {
		...
		if err != nil {
			w.logger.Error("Error notifying change", "error", err)
		}
		return
	}
	...
	if err := w.notifyFileEvent(ctx, uri, changeType); err != nil {
		w.logger.Error("Error notifying LSP server about file event", "error", err)
	}
}
...
func (w *WorkspaceWatcher) notifyFileEvent(ctx context.Context, uri string, changeType protocol.FileChangeType) error {
	cnf := w.configProvider.Get()
	if cnf.DebugLSP {
		w.logger.Debug("Notifying file event",
			"uri", uri,
...
func (w *WorkspaceWatcher) shouldExcludeFile(filePath string) bool {
	...
	if info.Size() > maxFileSize {
		if cnf.DebugLSP {
			w.logger.Debug("Skipping large file",
				"path", filePath,
				"size", info.Size(),
				"maxSize", maxFileSize,
				"debug", cnf.Debug,
...
func (w *WorkspaceWatcher) openMatchingFile(ctx context.Context, path string) {
	...
	if watched, _ := w.isPathWatched(path); watched {
		...
		if isHighPriorityFile(path, serverName) {
			if cnf.DebugLSP {
				w.logger.Debug("Opening high-priority file", "path", path, "serverName", serverName)
			}
			if err := w.client.OpenFile(ctx, path); err != nil && cnf.DebugLSP {
				w.logger.Error("Error opening high-priority file", "path", path, "error", err)
			}
			return
		}
		...
		if shouldPreloadFiles(serverName) {
			...
			if info.Size() > (1 * 1024 * 1024) { // 1MB limit for preloaded files
				if cnf.DebugLSP {
					w.logger.Debug("Skipping large file for preloading", "path", path, "size", info.Size())
				}
				return
			}
