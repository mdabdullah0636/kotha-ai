...
func NewClient(ctx context.Context, command string, args ...string) (*Client, error) {
	return NewClientWithConfig(ctx, nil, nil, command, args...)
}

func NewClientWithConfig(ctx context.Context, configProvider config.ConfigProvider, logger logging.Logger, command string, args ...string) (*Client, error) {
	if configProvider == nil {
		configProvider = config.GetDefaultProvider()
	}
	if logger == nil {
	...
	go func() {
		defer client.logger.RecoverPanic("LSP-message-handler", func() {
			client.logger.ErrorPersist("LSP message handler crashed, LSP functionality may be impaired")
		})
...
func (c *Client) InitializeLSPClient(ctx context.Context, workspaceDir string) (*protocol.InitializeResult, error) {
	...
	// Register handlers
	c.RegisterServerRequestHandler("workspace/applyEdit", func(params json.RawMessage) (any, error) { return HandleApplyEdit(c, params) })
	c.RegisterServerRequestHandler("workspace/configuration", HandleWorkspaceConfiguration)
	c.RegisterServerRequestHandler("client/registerCapability", func(params json.RawMessage) (any, error) { return HandleRegisterCapability(c, params) })
	c.RegisterNotificationHandler("window/showMessage", func(params json.RawMessage) { HandleServerMessage(c, params) })
...
func (c *Client) WaitForServerReady(ctx context.Context) error {
	...
	if cnf.DebugLSP {
		c.logger.Debug("Waiting for LSP server to be ready...")
	}
	...
	if serverType == ServerTypeTypeScript {
		if cnf.DebugLSP {
			c.logger.Debug("TypeScript-like server detected, opening key configuration files")
		}
	...
	for {
		...
		case <-ticker.C:
			...
			if err == nil {
				...
				if cnf.DebugLSP {
					c.logger.Debug("LSP server is ready")
				}
				return nil
			} else {
				c.logger.Debug("LSP server not ready yet", "error", err, "serverType", serverType)
			}
...
func (c *Client) openKeyConfigFiles(ctx context.Context) {
	workDir := c.configProvider.WorkingDirectory()
	serverType := c.detectServerType()
	...
	for _, file := range filesToOpen {
		if _, err := os.Stat(file); err == nil {
			...
			if err := c.OpenFile(ctx, file); err != nil {
				c.logger.Debug("Failed to open key config file", "file", file, "error", err)
			} else {
				c.logger.Debug("Opened key config file for initialization", "file", file)
			}
		}
	}
}
...
func (c *Client) pingTypeScriptServer(ctx context.Context) error {
	...
	if err != nil {
		c.logger.Debug("Error walking directory for TypeScript files", "error", err)
	}
...
func (c *Client) openTypeScriptFiles(ctx context.Context, workDir string) {
	...
	err := filepath.WalkDir(workDir, func(path string, d os.DirEntry, err error) error {
		...
		if ext == ".ts" || ext == ".tsx" || ext == ".js" || ext == ".jsx" {
			...
			if err := c.OpenFile(ctx, path); err == nil {
				filesOpened++
				if cnf.DebugLSP {
					c.logger.Debug("Opened TypeScript file for initialization", "file", path)
				}
			}
		}
	...
	if cnf.DebugLSP {
		c.logger.Debug("Opened TypeScript files for initialization", "count", filesOpened)
	}
}
...
func (c *Client) CloseFile(ctx context.Context, filepath string) error {
	...
	if cnf.DebugLSP {
		c.logger.Debug("Closing file", "file", filepath)
	}
...
func (c *Client) CloseAllFiles(ctx context.Context) {
	...
	for _, filePath := range filesToClose {
		...
		if err != nil && cnf.DebugLSP {
			c.logger.Warn("Error closing file", "file", filePath, "error", err)
		}
	}

	if cnf.DebugLSP {
		c.logger.Debug("Closed all files", "files", filesToClose)
	}
}
