...
func (b *mcpTool) Run(ctx context.Context, params tools.ToolCall) (tools.ToolResponse, error) {
	...
	p := b.permissions.Request(
		permission.CreatePermissionRequest{
			SessionID:   sessionID,
			Path:        b.configProvider.WorkingDirectory(),
...
func NewMcpTool(name string, tool mcp.Tool, permissions permission.Service, mcpConfig config.MCPConfig, configProvider config.ConfigProvider, logger logging.Logger) tools.BaseTool {
	return &mcpTool{
		mcpName:        name,
		tool:           tool,
		mcpConfig:      mcpConfig,
...
func getTools(ctx context.Context, name string, m config.MCPConfig, permissions permission.Service, c MCPClient, configProvider config.ConfigProvider, logger logging.Logger) []tools.BaseTool {
...
func GetMcpTools(ctx context.Context, permissions permission.Service, configProvider config.ConfigProvider, logger logging.Logger) []tools.BaseTool {
	if len(mcpTools) > 0 {
		return mcpTools
	}
	for name, m := range configProvider.Get().MCPServers {
		switch m.Type {
		case config.MCPStdio:
			c, err := client.NewStdioMCPClient(
				m.Command,
			...
			mcpTools = append(mcpTools, getTools(ctx, name, m, permissions, c, configProvider, logger)...)
		case config.MCPSse:
			c, err := client.NewSSEMCPClient(
				m.URL,
