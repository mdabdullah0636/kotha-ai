...
func TestLsTool_Run(t *testing.T) {
	...
	config.SetDefaultProvider(config.NewDefaultProvider(cfg))
	defer config.SetDefaultProvider(nil)
	...
	t.Run("handles temp directory", func(t *testing.T) {
		tool := NewLsTool()
		params := LSParams{
			Path: tempDir,
		}
...
func TestShouldSkip(t *testing.T) {
	testCases := []struct {
		...
		{
			name:           "node_modules nested path",
			path:           "/path/to/node_modules/package",
			ignorePatterns: []string{},
			expected:       false, // base name is "package", not "node_modules"
		},
		{
			name:           "node_modules root",
			path:           "/path/to/node_modules",
			ignorePatterns: []string{},
			expected:       true, // base name matches "node_modules"
		},
		{
			name:           "normal file",
