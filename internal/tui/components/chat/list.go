...
func (m *messagesCmp) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case pubsub.Event[session.Session]:
		if msg.Type == pubsub.EventTypeUpdated && msg.Payload.ID == m.session.ID {
	...
	case pubsub.Event[message.Message]:
		needsRerender := false
		if msg.Type == pubsub.EventTypeCreated {
		...
		} else if msg.Type == pubsub.EventTypeUpdated && msg.Payload.SessionID == m.session.ID {
		...
		if needsRerender {
			...
			if len(m.messages) > 0 {
				if (msg.Type == pubsub.EventTypeCreated) ||
					(msg.Type == pubsub.EventTypeUpdated && msg.Payload.ID == m.messages[len(m.messages)-1].ID) {
					m.viewport.GotoBottom()
				}
			}
		}
