...
func WriteMessage(w io.Writer, msg *Message, configProvider config.ConfigProvider, logger logging.Logger) error {
	data, err := json.Marshal(msg)
	if err != nil {
		return fmt.Errorf("failed to marshal message: %w", err)
	}
	cnf := configProvider.Get()
...
func ReadMessage(r *bufio.Reader, configProvider config.ConfigProvider, logger logging.Logger) (*Message, error) {
	cnf := configProvider.Get()
	// Read headers
...
func (c *Client) handleMessages() {
	...
	for {
		msg, err := ReadMessage(c.stdout, c.configProvider, c.logger)
		if err != nil {
			if cnf.DebugLSP {
		...
		if msg.Method != "" && msg.ID != 0 {
			if cnf.DebugLSP {
				c.logger.Debug("Received request from server", "method", msg.Method, "id", msg.ID)
			}
			...
			if err := WriteMessage(c.stdin, response, c.configProvider, c.logger); err != nil {
				c.logger.Error("Error sending response to server", "error", err)
			}
		...
		if msg.Method != "" && msg.ID == 0 {
			...
			if ok {
				if cnf.DebugLSP {
					c.logger.Debug("Handling notification", "method", msg.Method)
				}
				go handler(msg.Params)
			} else if cnf.DebugLSP {
				c.logger.Debug("No handler for notification", "method", msg.Method)
			}
			continue
		}
		...
		if msg.ID != 0 && msg.Method == "" {
			...
			if ok {
				if cnf.DebugLSP {
					c.logger.Debug("Received response for request", "id", msg.ID)
				}
				ch <- msg
				close(ch)
			} else if cnf.DebugLSP {
				c.logger.Debug("No handler for response", "id", msg.ID)
			}
		}
	}
}
...
func (c *Client) Call(ctx context.Context, method string, params any, result any) error {
	...
	if cnf.DebugLSP {
		c.logger.Debug("Making call", "method", method, "id", id)
	}
	...
	// Send request
	if err := WriteMessage(c.stdin, msg, c.configProvider, c.logger); err != nil {
		return fmt.Errorf("failed to send request: %w", err)
	}

	if cnf.DebugLSP {
		c.logger.Debug("Request sent", "method", method, "id", id)
	}
	...
	if cnf.DebugLSP {
		c.logger.Debug("Received response", "id", id)
	}
